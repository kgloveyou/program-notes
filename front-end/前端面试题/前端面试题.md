...
vue $nextTick的具体实现，要具体到vue2和vue3用了哪些微任务和宏任务来实现异步更新，这些微/宏任务的特点。（币安二面）
...
- Expires（早期使用；如果 `cache-control 与 expires` 同时存在的话， `cache-control` 的优先级高于 `expires`）
...
7. webpack 热更新原理
   ...
   [webpack 热更新原理解析](https://www.jianshu.com/p/d37475f1bae1)
...
2、babel中的preset和plugin的区别

Babel是代码转换器，将es6转化为es5，将JSX转换为JS。

#### Babel Preset：

Babel插件一般尽可能拆成小的粒度，开发者可以按需引进。这样的好处显而易见，既提高了性能，也提高了扩展性。可以简单的把Babel Preset视为Babel Plugin的集合。比如babel-preset-es2015就包含了所有跟ES6转换有关的插件。
...
作者：亦晓寒
链接：https://www.jianshu.com/p/4cfacd3538f2

webpack中的配置：

annot-ui\webpack.config.js
...
​	Git hooks 是在事件之前或之后执行的脚本，例如：提交、推送和接收。 Git hooks 是一个内置功能 - 无需下载任何东西。 Git hooks 在本地运行。
...
[monorepo大型前端多项目管理方案及lerna+yarn workspace实践](https://segmentfault.com/a/1190000040186587)
...
POST请求的两种编码格式：`application/x-www-urlencoded`是浏览器默认的编码格式，用于键值对参数，参数之间用`&`间隔；`multipart/form-data`常用于文件等二进制，也可用于键值对参数，最后连接成一串字符传输(参考Java OK HTTP)。
...
[前端安全系列（一）：如何防止XSS攻击？](https://juejin.cn/post/6996948716041011208#heading-6)

[Web安全之SQL注入攻击技巧与防范（转）](https://www.cnblogs.com/ajianbeyourself/p/13298510.html)
...
[Blob Base64 File等前端文件流处理](https://juejin.cn/post/7036594118960611364)

[Base64 格式](https://c.runoob.com/front-end/59/)
