...
?? 操作符用于提供一个默认值，但仅当左侧的值为 null 或 undefined 时才使用默认值。这与传统的逻辑或操作符 || 不同，后者会在左侧值为“假值”（falsy）时就返回右侧值。
...
​	在async 函数体内，await 之前的代码是同步执行的，可以理解为是给promise构造函数传入的代码。await 之后的所有代码都是在promise.then 中的回调，会被推入微任务队列中。
