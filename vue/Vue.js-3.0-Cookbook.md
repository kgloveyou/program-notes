...
安全一直是每个人都担心的事情，技术也不例外。 你需要时刻保持警觉和警觉。 在本节中，我们将了解如何使用一些技术和简单的解决方案来防止攻击。

### Linters  

### JavaScript  

以下是使用 JavaScript 的一些提示：

- 如果要存储 API token，请始终使用 `SessionStorage`。
- 始终捕获错误并且不向用户显示任何堆栈跟踪，以防止任何代码操作。

以下是一些使用 JavaScript 时不要做的事情的提示：

- 切勿将 API token 存储在 `LocalStorage` 上。
- 切勿将敏感数据存储在 JWT 对象中。
- 
...
- 始终为你的 props 添加类型验证，如果可能的话，进行验证器检查。

- 避免组件的全局注册； 使用局部组件。

- 尽可能使用延迟加载组件。

- 使用 `$refs` 而不是直接的 DOM 操作。

以下是一些使用 Vue 时不要做的事情的提示：

- 切勿在 window 或任何全局范围内存储 Vue、$vm、$store 或任何应用程序变量。

- 永远不要修改 Vue 原型； 如果您需要向原型添加新变量，请制作一个新的 Vue 插件。

- 不建议在组件之间使用直接连接，因为这会使组件绑定到父组件或子组件。
