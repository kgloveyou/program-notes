...
2、服务器向客户端确认收到发来的序列号（ACK），并发送一个服务器同步消息（SYN），告诉客户端它要使用的序列号。 两个消息组合成一个 SYN-ACK 消息。
...
​		HTTP/2的流会经过一些生命周期的状态。客户端发送HEADERS  帧以开启一个HTTP请求（比如GET请求），服务器响应此请求，然后流结束。这个过程经历如下状态：
...
​		H2spec是一个HTTP/2一致性测试工具，它发送不同的消息到HTTP/2服务器，并检查服务器是否正确遵循规范。下载适合你的版本，然后找一个HTTP/2服务器做测试：
