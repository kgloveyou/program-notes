...
解决办法就是用一张`Canvas`填充整个视口背景，并利用一些其他元素来代表每个“虚拟画布”`(virtual canvas)`，即只在一张`Canvas`中加载一个`Renderer`，并为每个`virtual canvas`创建一个场景`(Scene)`。这样我们只需要确保每个`virtual canvas`正确的位置，THREE.js就会将它们渲染在屏幕上相应的位置。
...
最终效果如下：
![CPT2111201437-851x454.gif](https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/fb3fe63a37f94b8f9b0205ac59bc1318~tplv-k3u1fbpfcp-watermark.image?)
<p align="center"><a herf = "https://codepen.io/deomo/pen/rNzRZpx" target = "_blank">点击在新标签页中查看效果</a></p>

可以看到，两个物体被分别渲染到了对应的位置。

## 同步滚动

虽然我们已经实现了同时渲染多个场景的功能，但是上面的代码依然存在一个问题，如果`Scene`过于复杂、或者由于其他原因需要更长时间渲染，那么画布中`Scene`渲染的位置总是会落后于页面的其他元素，如页面滚动时会出现明显的滞后。
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/9b705893567b49e2bd36da421c106d28~tplv-k3u1fbpfcp-zoom-1.image)
...
接着，我们利用`transform`属性来移动画布，让画布的顶部始终与页面的顶部对齐。
...
```
`position: fixed` 会完全禁用画布的滚动，无论其他元素是否已经滚动到它的上方；
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/5b54da578d71442dbcca0eb732947721~tplv-k3u1fbpfcp-zoom-1.image)

## 封装

现在，我们已经实现了在一个`Canvas`中渲染多个场景的功能，接下来就来处理一下让它更加好用些。

我们可以封装一个主渲染函数用来管理整个`Canvas`,并定义一个场景元素列表和他们对应的场景初始化函数。对于每个元素，它将检查该元素是否滚动到了可视区域并调用相应的场景初始化函数。这样我们就构建了一个渲染系统，在这个系统中每个独立的`scene`都会在它们各自定义的空间内独立渲染且不互相影响。
...
![CPT2111201549-769x454.gif](https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/1a0de3d4befa45b49b3cd90bbd8c67d3~tplv-k3u1fbpfcp-watermark.image?)
<p align="center"><a herf = "https://codepen.io/deomo/pen/porYxeN" target = "_blank">点击在新标签页中查看效果</a></p>
...
![CPT2111201620-722x421.gif](https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/bfe50c229e2e4db68e7d490aff649c30~tplv-k3u1fbpfcp-watermark.image?)
<p align="center"><a herf = "https://codepen.io/deomo/pen/WNEmaao" target = "_blank">点击在新标签页中查看效果</a></p>

上面提到的方法在这个网站上可以找到很多实例，比如[Three.js 图元](https://threejsfundamentals.org/threejs/lessons/threejs-primitives.html)和[Three.js 材质](https://threejsfundamentals.org/threejs/lessons/threejs-materials.html) 这两篇文章。
...
还有一种方法是利用[`OffscreenCanvas`](https://developer.mozilla.org/en-US/docs/Web/API/OffscreenCanvas)方法，但是截至2020年7月，只有Chrome支持这种方法，感兴趣的小伙伴可以点击查看文档。

翻译于：[`Three.js Multiple Canvases Multiple Scenes`](https://threejsfundamentals.org/threejs/lessons/threejs-multiple-scenes.html)
