...
目前这个功能运行良好。但仍有几个因素需要考虑——当前的折扣是针对每个产品单独设置的，例如披萨有它自己的折扣规则，意面也有它自己的。那么，如果我们想要实施一项全店范围的折扣（比如公共假日的折扣），我们该如何处理呢？

### 探索策略模式

假设这是一个繁忙的周五晚上，我们想对所有披萨和饮料提供特别折扣。但我们不打算对已有折扣的商品叠加额外优惠——比如，一份加了四款配料的披萨应该只享受这份特定的特别折扣。  

处理这种任意组合的折扣可能会比较复杂，这就需要将**折扣计算逻辑**与**商品类型**解耦。此外，我们还希望灵活地在周五结束后，或某个特定时间段后取消这些折扣。  

我们可以使用一种叫做**策略模式（Strategy Pattern）**的设计模式来实现这种灵活性。策略模式是一种**行为型设计模式**，它允许在**运行时选择算法的具体实现**。  该模式封装了一组算法，使它们可以互相替换，让客户端无需修改代码就能选择最合适的算法。



这种方法让我们的折扣逻辑保持模块化且易于调整，能够以最小的代码改动适应不同场景。随着我们将新的逻辑组件——钩子（Hooks）、数据模型、领域逻辑（折扣策略）和视图（Views）——从应用代码中抽离出来，整个应用正逐渐演变成一个分层的前端架构。

## 深入探讨分层架构

### 应用程序的分层结构

我们已深入探讨了如何将组件和模型拆分到不同文件中；持续优化项目结构同样至关重要。具有不同职责的功能模块应当存放在不同的文件夹中，这样可以简化应用导航流程并节省时间。目前，我们的应用程序呈现出全新的结构形态：
...
分层结构就此形成。在视图层，我们主要使用纯TSX渲染基础标签，这些视图通过Hooks进行状态和副作用管理。与此同时，在模型层，模型对象包含业务逻辑、切换不同折扣策略的算法以及数据格式转换等功能。这种结构实现了关注点分离，使代码更有条理、可复用且更易于维护。

需要注意的是这里存在单向依赖关系：上层可以访问下层，但反之则不行。TSX使用Hooks进行状态管理，而Hooks调用模型进行计算。但我们不能在模型层中使用JSX或Hooks。这种分层技术使得底层发生变更或替换时不会影响上层，从而形成清晰且易维护的结构。

在我们的Code Oven应用中（如图11.9所示），界面布局左侧是菜单项列表，右侧是购物车。在购物车中，每个商品都会在页面上显示详细的折扣和价格信息。

### 分层架构的优势

- 更强的可维护性：将组件划分为独立模块有助于更轻松地定位和修复特定代码段中的缺陷，从而减少修改耗时，并降低修改过程中引入新错误的可能性。

- 更高的模块化程度：该架构本身具有更强的模块化特性，能促进代码复用并简化新功能的添加。即使在视图等各层内部，代码也往往更具组合性。

- 更佳的可读性：代码逻辑变得更加清晰易懂、便于导航，这不仅对原始开发者有益，也有利于其他可能接触代码库的人员。这种清晰度是实现代码变更的关键。

- 更优的可扩展性：每个模块复杂度的降低使应用更具扩展性，更容易引入新功能或进行改动而不影响整个系统——这对于需要长期演进的大型复杂应用而言是关键优势。

- 技术栈迁移：尽管大多数项目中不太可能需要，但如果确有需求，由于领域逻辑被封装在独立于视图的纯JavaScript（或TypeScript）代码中，可以替换视图层而不改变底层模型和逻辑。

## 本章小结

在本章中，我们通过在应用中实施分层架构，显著提升了代码的可维护性、模块化程度、可读性、可扩展性以及技术栈迁移潜力。通过逻辑分离、利用自定义Hook优化ShoppingCart组件，以及将应用组织为独立层级，我们大幅强化了代码结构和管理便利性。这种架构方法不仅优化了现有代码库，更为未来的功能扩展和持续改进奠定了坚实基础。

下一章中，我们将完整展示从零开始实现应用的端到端流程：采用用户验收测试驱动开发方法，进行重构与持续清理，始终确保代码保持最佳整洁状态。
...
由于我们使用城市名称作为搜索查询条件，通常会在搜索结果中遇到多个匹配项。为了优化这一点，我们可以为每个结果项添加更多详细信息（例如州名、国家名甚至坐标），从而使结果更具区分度。  

遵循 TDD（测试驱动开发）方法，我们将从编写一个测试开始。虽然可以编写 Cypress 测试，但这类细节更适合用单元测试等更低层级的测试来覆盖。Cypress 测试属于端到端测试，涵盖所有部分——页面、网络（即使使用了拦截器）——从它的视角来看，它看不到组件，只看到 HTML、CSS 和 JavaScript。这使得它的执行成本更高，相比更低层级的测试（通常在内存浏览器中运行，并专注于隔离区域），反馈周期也更长。  

针对这一增强功能，我们将采用 Jest 测试，它更轻量、更快，并且在编写测试用例时能提供更高的针对性。  

在下面的代码片段中，我们的目标是测试某个结果项是否显示了城市名称。
...
我们当前的 App 组件代码过长，影响了可读性和功能扩展性，这表明需要进行一些重构来整理代码。该组件未能很好地遵循单一职责原则，因为它承担了多项职责：处理城市搜索和天气查询的网络请求、管理下拉菜单的开启/关闭状态，以及多个事件处理函数。

一种符合更好设计原则的方法是将UI拆分为更小的组件。另一种方法是使用自定义Hooks进行状态管理。鉴于此处的逻辑很大一部分围绕城市搜索下拉框的状态管理展开，从隔离这部分逻辑入手是明智的。
...
作为天气应用的最后一项功能，我们的目标是保留用户的选择，这样当他们下次打开应用时，看到的不是空列表，而是之前选中的城市。这个功能可能会被高频使用——用户只需初期添加几个城市，之后每次打开应用时，系统就能自动加载这些城市的天气信息。
...
在 hydrate 函数中，首先从 localStorage 的 favoriteItems 键下获取一个字符串化的数组，将其解析回 JavaScript 数组；如果该键不存在，则默认使用空数组。接着，遍历这个 items 数组，为每个元素创建一个新的 SearchResultItemType 实例，并将其传入 fetchCityWeatherData 函数。该函数返回一个 Promise，所有 Promise 被收集到一个 Promise 数组中。

然后使用 Promise.all 等待所有这些 Promise 完成解析后，再通过 setCities 更新状态，将获取到的城市天气数据填充进去。最后，在 useEffect 中调用 hydrate，以便在组件挂载时执行这一逻辑。

最后，当用户点击某个项目时，将其保存到 localStorage 中。
...
这里的 onItemClick 函数接收一个类型为 SearchResultItemType 的参数 item。在该函数内部，使用了延迟时间为 0 毫秒的 setTimeout，其本质是将其中的内容的执行推迟到当前调用栈清空之后 —— 这样可以避免阻塞 UI。
在这个被延迟执行的代码块中，它从 localStorage 的 favoriteItems 键下获取一个字符串化的数组，将其解析回 JavaScript 数组；如果该键不存在，则默认使用空数组。接着，它基于传入的 item 参数创建了一个新对象 newItem，并从中提取并重命名了部分属性。

随后，它将包含 newItem（位于数组开头）以及先前已存储项目的字符串化数组，更新到 localStorage 的 favoriteItems 键中。

在 setTimeout 外部，它调用了 fetchCityWeather 并传入 item 参数，用于获取被点击城市的天气数据，并将此次调用的结果作为 onItemClick 的返回值返回。

## 本章总结

在本章中，我们从零开始创建了一个天气应用，并严格遵循测试驱动开发（TDD）方法论。我们使用 Cypress 进行用户验收测试，用 Jest 进行单元测试，逐步构建应用的各项功能。同时，还探讨了关键实践，例如领域对象建模、网络请求存根（stubbing）以及在重构过程中应用单一职责原则。

虽然本章并未涵盖前文所有技术细节，但它强调了开发阶段保持节奏严谨的重要性，突出了识别代码“异味”（smells）并有效解决的价值，同时确保充分的测试覆盖率，从而构建健壮且可维护的代码库。本章作为实践的综合演练，鼓励你运用所学知识与技术进一步优化应用。

在接下来的终章中，我们将回顾所探讨的反模式，重温已学习的设计原则与实践方法，并提供额外的学习资源供深入探索。

# 13 回顾反模式原则

本书讨论的技术可能并不具有突破性或新颖性；事实上，其中许多技术早已成熟。然而，这些技术在React生态系统中的应用却仍显不足。我衷心希望本书能够成功弥合这一差距，将这些宝贵的设计原则和模式重新引入React社区，从而为开发者长期带来更顺畅的编码体验。
...
在前面的章节中，我们探讨了诸多反模式。识别反模式是修正它的第一步。下面，让我们简要回顾一下迄今为止所学的内容。

### Props 逐层传递（Props Drilling）

当某个 prop 需要穿过多个组件层级，最终仅在深层组件中使用时，就会产生 Props 逐层传递 问题。这会导致中间组件被迫知晓本不必要的 prop，使得代码变得复杂且难以维护。

✅ 解决方案：使用 Context API 创建一个中央存储（store）及访问该存储的函数，使组件树在需要时能获取 props，而无需逐层传递。

### 过长的 Props 列表 / 过大的组件（Long Props List / Big Component）

如果一个组件接收大量 props，或者包含过多逻辑，就会变成一个难以理解、复用或维护的“庞然大物”。这种反模式违反了 单一职责原则（SRP）——即一个组件或模块应该只有一个变更的理由。

✅ 解决方案：拆分组件：将大组件分解成更小、更易管理的子组件，并分离关注点（segregate concerns）。单一职责：每个组件应只负责一个明确的任务。自定义 Hook：使用 Custom Hooks 可以简化组件内部的逻辑，减少组件体积。

### 业务逻辑泄漏

当业务逻辑被植入本应纯展示的组件时，就会发生业务逻辑泄漏，这会加大应用管理难度并降低组件复用性。
解决方案：通过自定义Hook抽离业务逻辑，或将其迁移至独立模块/层，以此分离展示逻辑与业务逻辑。采用ACL也是有效的解决方法。

### 视图中的复杂逻辑

在视图组件中嵌入复杂逻辑会使代码混乱，导致难以阅读、理解和维护。视图应尽可能保持简洁，只负责数据渲染。
解决方案：将复杂逻辑迁移至自定义Hook、工具函数或独立业务逻辑层，以保持视图组件简洁可控。建议先拆分组件，再逐步将逻辑分离至适当位置。

### 各层级测试缺失

若缺乏足够的单元测试、集成测试或端到端测试来验证应用功能，可能引发Bug、回归问题，并导致代码难以重构或扩展。
解决方案：采用包含单元测试、集成测试和端到端测试的完善测试策略，并结合TDD等方法，以确保代码正确性和可维护性。

### 代码重复

在多个组件或应用的不同部分重复相似代码，会增加代码库维护难度并提高出现 Bug 的概率。
解决方案：遵循 "不要重复自己"（DRY） 原则，将通用功能抽象为共享的工具函数、组件或自定义 Hook，从而减少代码重复并提升代码可维护性。

剖析完常见反模式后，现在有必要深入探讨那些能解决这些普遍问题的设计原则。这些原则不仅能提供解决方案，还能指导你写出更简洁、高效的代码。

## 设计模式速览

React 中存在一些能有效应对反模式的设计模式，有趣的是，其中部分模式的应用范围超越了 React 本身，在更广泛的场景中同样适用。下面让我们快速回顾这些模式。

### 高阶组件（Higher-Order Components, HOCs）

高阶组件是 React 中用于复用组件逻辑的强大模式。它是一种接受组件并返回增强版新组件的函数，能为组件追加额外属性或行为。通过使用高阶组件，你可以提取并共享通用逻辑，从而缓解诸如 Props 逐层传递 和 代码重复 等问题。

### Render Props 模式

Render Props 是一种通过将函数作为 prop 传递来实现组件间代码共享的技术。具体来说，就是向组件传递一个返回 React 元素的函数作为 prop。该模式通过促进逻辑复用与组合，能有效改善 过长的 Props 列表 和 庞大的组件体积 问题。

### 无头组件（Headless Components）

无头组件负责管理行为和逻辑，但不渲染 UI，将渲染控制权完全交给使用者。它们将行为逻辑与展示逻辑分离，能有效解决 业务逻辑泄漏 和 视图中的复杂逻辑 问题，使组件更加灵活且易于维护。

### 数据建模（Data Modeling）

数据建模是指对数据进行组织和定义，有助于理解和管理应用内的数据，从而简化组件内的逻辑。该原则可用于应对 视图中的复杂逻辑 和 业务逻辑泄漏 问题。

### 分层架构

分层架构通过分离关注点并组织代码，使每一层都具有明确的职责。这种分离方式能让代码库更加有序且易于管理，从而有效解决业务逻辑泄漏和视图中的复杂逻辑等问题。

需要提醒的是，图13.1展示了这种分层架构。在该架构中，每一层都包含多个模块，每个模块负责整个应用中特定的任务。其中包括用于数据获取的模块（如图13.1所示的Fetcher模块）、用于对接外部服务（如社交媒体登录和支付网关）的适配器（如图13.1所示的Adaptor网关），以及用于分析功能和与安全相关功能的组件。

### 上下文即接口

将上下文（Context）作为接口使用，能让组件无需通过多层 Props 逐级传递即可访问数据。该策略能有效缓解 Props 逐层传递（Props Drilling） 和 冗长的 Props 列表 问题，使组件树更清晰易维护。

在扎实掌握基础原理与设计原则后，接下来我们将探讨一些实用技巧，帮助你在日常编码中运用这些原则。

## 重温基础设计原则

除了 React 特有的模式外，我们在各章节还讨论了多个更高层次的设计原则。这些原则不仅适用于 React 开发，还能指导数据建模、事件测试或集成脚本等各类工作场景。它们不受特定上下文限制，掌握这些原则能显著提升你在不同领域的编码水平。

### **单一职责原则（SRP）**

单一职责原则主张一个类或组件应该只有**一个变更理由**。遵循该原则可以写出更易维护、更清晰的代码，有效缓解**大组件**和**视图逻辑复杂**等问题。

我们已在多个层面实践过这一原则：从拆分大组件为小组件，到创建新 Hook，再到重大重构（比如为天气应用集成 ACL）。值得注意的是，当你陷入大组件的泥潭时，单一职责原则永远是最可靠的帮手。

### **依赖倒置原则（DIP）**

依赖倒置原则（DIP）强调要依赖**抽象而非具体实现**，从而实现高层模块与低层模块的解耦。该原则能有效管理**业务逻辑泄漏**，并促进**关注点分离（SoC）**。

### **不要重复自己（DRY）**

DRY 原则的核心是减少代码重复。遵循这一原则可以降低代码冗余，让你的代码库更易于维护和扩展。

### 防腐层（ACL）

防腐层（ACL）作为应用程序不同部分或层级之间的屏障，能够创建稳定的接口。实施防腐层是管理业务逻辑泄漏、确保**关注点清晰分离（SoC）**的有效策略。

当你的代码需要以任何形式与其他系统交互时（这种场景在与不同团队协作时尤为常见——这也是许多开发环境中的常态），防腐层会发挥显著作用。通过ACL建立清晰的系统边界，我们可以降低其他系统变更对本系统的影响，从而更好地掌控应用开发，并缓解潜在的集成难题。图13.2展示了防腐层在React中的应用方式。

<img src="React Anti-Patterns.assets/image-20250929173629233.png" alt="image-20250929173629233" style="zoom:50%;" />

### 运用组合模式

组合（Composition）是 React 的核心原则之一，它允许开发者通过其他组件来构建新组件，从而提升代码复用性和简洁性。采用组合模式可以有效缓解多种问题，包括冗长的 Props 列表、庞大的组件体积以及代码重复，最终构建出更易维护、结构更清晰的代码库。

理解这些反模式、设计模式和原则，对管理前端代码库的复杂性至关重要。但与此同时，具体的技术和实践同样重要——因为它们正是开发者日常工作中实际运用的内容。

## 技术与实践总结

我们特别强调了测试和渐进式改进的重要性。这种方法不仅能保持代码的高质量，还能培养全面发展的开发者——锤炼他们的批判性思维能力，并学会专注解决单个问题。

### 编写用户验收测试（UAT）

用户验收测试（UAT）是开发过程中的关键环节，它能确保应用程序符合需求规格，并按预期运行。实施 UAT 有助于在开发早期发现问题，保证应用始终朝着正确的方向推进。

如图 13.3 所示，我们特别强调：测试应当从最终用户的角度出发，聚焦于交付用户价值，而非纠结于实现细节。这一点在你从较高层级开始实现某个功能时尤为重要。

### 测试驱动开发
TDD是一种软件工程技术，其中需要在被测代码之前编写测试。该过程主要分为以下迭代开发周期：编写测试，使测试通过，然后重构。TDD可以显著帮助确保您的代码库功能正常且无错误，解决每个级别上缺乏测试的问题。

### 重构和常见代码异味
重构涉及在不改变外部行为的情况下增强现有代码的设计。了解常见代码异味并持续重构您的代码可以带来更健康、更可维护的代码库。该技术可以帮助解决诸如代码重复、视图中的复杂逻辑和业务泄漏等问题。

既然我们已经探讨了常见的反模式，阐明了设计原则并研究了相关技术，现在该超越本书的范围了。以下部分提供了一份推荐阅读清单，这将有助于您进一步加深对React、TypeScript和软件设计原则的理解，并提升相关技能。

## 总结

我想向您致以最诚挚的感谢，感谢您对磨练技艺和追求技术卓越的执着与热情。正是像您这样渴求知识、不断进步的人，推动着整个行业向前发展。当您翻过这最后一页时，请记住，这段旅程并未结束——事实上，它将在您的实际项目中开启新的探索与应用篇章。

成长的真谛在于学以致用，并持续努力将所学知识应用于挑战常规、追求更优方案。本书旨在为您提供坚实的基础，但真正的魔力在于：当您将这些概念付诸实践、亲身尝试，并融入日常工作之中。

衷心感谢您拨冗阅读本书内容并与之互动。我由衷希望您能保持这份势头，深入钻研，继续以您的贡献丰富React社区。在您踏上新征程之际，祝您一切顺利——愿您的代码简洁优雅，解决方案创新突破，整个历程收获满满。

感谢您，也祝愿您在持续学习与精进的道路上好运常伴！
