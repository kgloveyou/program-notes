...
分析器工具是 React 开发者工具套件中使用频率较低的一个工具。这个工具复杂得多，用于特定场景。它类似于浏览器内置开发者工具中自带的性能分析功能。

假设你有一个应用程序运行缓慢。当你点击某个特定按钮时，应用程序会冻结约半秒钟，然后才响应点击并执行更新操作。为什么会这样？这种情况正是分析器的用武之地。

使用分析器时，你可以在点击按钮前开始录制，在按钮触发操作过程中保持录制状态，操作完成后停止录制。然后你可以回放录制内容查看具体执行过程：哪些函数被调用、哪些组件重新渲染及其原因（如果启用了重渲染跟踪）、副作用函数的执行时机、组件上触发了哪些事件，以及许多其他细节。

这个工具的界面比较复杂，包含大量可能难以理解的信息。掌握它的最佳方式就是通过实践积累经验。尝试调试第4.1节中的深色模式应用：在分析器中开始录制，切换深色模式按钮，然后停止录制。结果应该会呈现为类似图4.16所示的界面。
...
# 13、项目：创建一个React UI 库

## 13.1 现有技术栈

### 13.1.1 Storybook：可视化测试

假设你已经拥有一个UI组件库，现在需要用它来实现一个新功能。你拿到了一份设计稿，其中包含不同类型的按钮和输入框。你知道这些组件都已存在于你的库中，但该如何找到它们？又如何确定哪个组件的哪个配置参数能实现设计稿中的效果？

这就是可视化测试工具发挥作用的地方。Storybook可能是同类工具中独一无二的选择，许多团队都已采用它。它特别适合UI库的可视化展示。

简单来说，Storybook是一种可视化浏览代码库的方式。你可以直观看到每个组件在不同属性组合和输入配置下的呈现效果。如果配置得当，你甚至可以实时调试组件来观察它们的交互方式。同时，你还能查看每个组件可视化展示背后的源代码。对于本章将要使用的自定义UI库，其Storybook的输出效果如图13.1所示。

### 13.1.2 Istanbul：代码覆盖率报告

关于单元测试的一个重要问题是：多少测试才算足够？这个问题有很多答案，其中一个简单的标准是：当代码的每一部分都经过测试时。那么，如何判断哪些代码行或行中的哪些部分被测试覆盖了呢？这里的关键术语就是代码覆盖率。代码覆盖率报告工具能够（通过计算机科学的魔力）显示在运行整套测试时，代码的哪些部分被执行了多少次。Istanbul 就是这样一个工具，它是 JavaScript 和 TypeScript 中最流行的代码覆盖率工具之一。

假设我们有这样一个函数，它会返回一个人的显示名称，并附带一些可选的额外信息：
...
我们覆盖了多少代码呢？我们没有测试当 if 语句为 true 时会发生什么，也没有测试当 prefix 缺失时会发生什么。如果我们启用代码覆盖率运行这个测试，Istanbul 会高亮显示那些未被执行的代码部分（图 13.3）。

在图13.3截图的顶部，我们可以看到一份覆盖率报告摘要。Istanbul报告显示：在全部3条语句中只执行了2条，3个分支中只走了1个，3行代码中只有2行被实际评估。不过所有函数都被调用了，这算是一个小胜利。
...
追求神奇的100%代码覆盖率往往徒劳无功，特别是当你试图为整个项目达成这一目标时。但针对特定文件、文件夹或软件包实现100%覆盖率可能是值得的，尤其是当它们包含被整个应用程序广泛使用的通用函数或工具，或是涉及特别密集或复杂的数学运算时。
